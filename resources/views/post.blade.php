<x-base-layout :title="$post->title">
    ...
    <div class="mb-10 leading-loose lg:mb-12 px-5 italic py-2 border-l-4 text-gray-800 dark:text-white border-gray-600 dark:border-white">"{{ $post->excerpt }}"</div>
    <div class="text-gray-800 dark:text-white" id="post-content">
        {!! $post->content !!}
    </div>
    <div id="about-author" class="text-gray-800 dark:text-white lg:max-w-[70%] mt-7 mb-10">
        <h2 class="font-bold mb-3 md:mb-5 uppercase">Tentang Penulis</h2>
        <div class="flex items-center gap-4" id="auhtor-bio">
            ...
            <div class="flex flex-col">
                <a href="{{ route('author.posts', $post->author->uuid) }}"
                    class="font-semibold truncate block">{{ $post->author->name }}</a>
                <a href="mailto:{{ $post->author->email }}"
                    class="text-xs block">{{ $post->author->email }}</a>
                <p class="text-xs lg:text-sm mt-3">{{ $post->author->profile->bio }}</p>
            </div>
        </div>
    </div>
</x-base-layout>
